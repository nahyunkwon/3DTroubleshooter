...
def index():
    ...
    if request.method == 'POST':
        ...
        for file in request.files.getlist("img-file"):
            ...
            if not file:
                ...
                for m in model_info.index:
                    ## Load all models
                    model = torch.load("static/model/" + model_info.loc[m, "filename"],
                    ...
                    df_results = df_results.append(
                        pd.Series((model_info.loc[m, "name"], b, str((img_max_w, img_max_h)), d), index=output_vars),
                        ignore_index=True)
                ...
                if sum(df_results.top1prob_has_err >= acc_threshold) > 1000:  # 0:
                    ...
                    card_info_prob.append(
                        df_sub.sort_values(by="top1prob_has_err", ascending=False).top1prob_has_err.to_list())

                else:
                    ## Store error info for cards
                    card_info.append(df_results.sort_values(by="top1prob_has_err", ascending=False).err_name.to_list())
                    card_info_prob.append(
                        df_results.sort_values(by="top1prob_has_err", ascending=False).top1prob_has_err.to_list())
        ...
        if len(img_path) > 0:
            return render_template("index.html", img_format='data:image/JPEG;base64,',
                                   svg_format='data:image/SVG+xml;base64,', img_path=img_path, n_img=len(img_path),
                                   output=text + str(len(img_path)) + " file(s) -- " + text_names, card_info=card_info,
                                   init_hide=False,
